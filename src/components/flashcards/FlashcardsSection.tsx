...
export function FlashcardsSection({ topicId }: FlashcardsSectionProps) {
  ...
  const handleUpdateFlashcard = async (flashcardData: { front: string; back: string }) => {
  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        <p className="mb-4 text-red-500">{error}</p>        <button 
  ...
  if (flashcards.length === 0) {
    return (
      <div className="p-8 text-center rounded-xl bg-background shadow-medium max-w-lg mx-auto">
        ...
        <h3 className="mb-2 text-2xl font-bold bg-gradient-to-r from-primary via-secondary-400 to-accent-200 text-transparent bg-clip-text">Brak fiszek</h3>
        <p className="mb-4 text-lg font-medium text-white">Nie masz jeszcze fiszek w tym temacie.</p>
        <p className="text-gray-400 mb-6">Dodaj swoje pierwsze fiszki lub wygeneruj je przy pomocy AI.</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">          <a 
            href={`/topics/${topicId}/create`}
            className="rounded-md px-4 py-2 bg-gradient-to-r from-primary via-secondary-400 to-accent-200 text-white font-medium shadow-light hover:shadow-medium duration-300 cursor-pointer hover:scale-110 transition-all"
          >
            Dodaj fiszki
          </a>          <a 
  ...
  return (
    <>
      ...
      <div className="flex justify-end mb-6 gap-3">        <a 
          href={`/topics/${topicId}/create`}
          className="rounded-md px-4 py-2 bg-transparent border border-gradient-to-r from-primary via-secondary-400 to-accent-200 text-white font-medium shadow-light hover:shadow-medium transition-shadow duration-300 cursor-pointer"
        >
          Dodaj fiszki
        </a>        <a 
          href={`/topics/${topicId}/generate`}
